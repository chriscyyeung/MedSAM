...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--input-dir", 
        type=str,
        help="Path to directory containing BUSI dataset."
    )
    parser.add_argument(
        "-c", 
        "--cls", 
        type=str,
        help="Class to preprocess (normal, benign, or malignant)."
    )
    parser.add_argument(
        "--train-output-dir", 
        type=str,
...
def main(args):
    ...
    imgs = glob.glob(os.path.join(args.input_dir, args.cls, "*[!mask].png"))
    imgs = [img for img in imgs if "(53)" not in os.path.basename(img)]
    ...
    if args.sanity_check:
        ...
        train_gt = np.load(os.path.join(train_gts_path, train_img_fn[:-4] + "_mask.npy"))
        ...
        test_gt = np.load(os.path.join(test_gts_path, test_img_fn[:-4] + "_mask.npy"))
        _, axs = plt.subplots(1, 2, figsize=(10, 5))
